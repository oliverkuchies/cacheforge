...
import { MemoryCacheLevel, RedisCacheLevel } from "./levels";
...
describe('Cache Service with multiple levels and versioning', () => {
    beforeAll(async () => {
        ...
        const memoryStrategy = new MemoryPercentageLimitStrategy(70, evictionPolicy);
        memoryLevel = new MemoryCacheLevel(memoryStrategy);
    ...
    it('should correctly store and retrieve versioned keys across cache levels', async () => {
        ...
        expect(redisValue, 'Redis cache should return the correct value').toBe(value);

        const memoryValue = await memoryLevel.get(cacheKey);

        expect(memoryValue, 'Memory cache should return the correct value').toBe(value);
    });
    ...
    it('should correctly store and retrieve versioned keys across cache levels- versioned', async () => {
        ...
        expect(cacheServiceValue, 'Versioned Cache Service should return the correct value').toBe(value);
    });
    ...
    it('should get data from the highest cache level available', async () => {
        ...
        expect(retrievedValue, 'Retrieved value should match the original value').toBe(value);
    ...
    it('should lock and unlock keys correctly across cache levels', async () => {
        ...
        const result = await cacheService.lock<number>(cacheKey, async () => {
            lockAcquired = true;
            return 99;
        }, 5);
    ...
    it('should throwing a locking exception if all levels do not support locking', async () => {
        ...
        const cacheKey = faker.string.alpha(10);
        await expect(simpleCacheService.lock<string>(cacheKey, async () => {
            return 'test';
